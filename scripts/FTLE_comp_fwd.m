...
infile = input('Input file name root? ');

dt = input('Time step for computing FTLE (in days, even)? ');


ncid = netcdf.open(['/Users/frederic/Documents/JOB/Projets/Krill/Modelling/FTLE/' infile '_FTLE.nc'],'nowrite');
...
mi = 0; ni = 0;     % Initial coordinates for the (sub)sampling grid for computing FTLEs

mf = m; nf = n;     % Final coordinates for the (sub)sampling grid
...
outfile = [infile '_FTLE_' num2str(dt) 'd'];
